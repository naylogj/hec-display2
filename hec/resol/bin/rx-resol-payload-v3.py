...
# Extended to send data to thingsboard
# 
# Version 3
...
thingsboard = True # send data to Thingsboard.io if True
#thingsboard = False # send data to Thingsboard.io if True
ACCESS_TOKEN = "your-token-string"	# token for HEC device in demo account
...
def got_msg(client, userdata, message):
    ...
    if thingsboard:
        send_to_thingsboard(_buffer)

# ---------------------------------------------------------------------
def send_to_thingsboard(_message):
	global ACCESS_TOKEN
	_tbhost = "demo.thingsboard.io"			# check this
		
	# form the payload in key value pairs [{"key1":"value1"}, {"key2":"value2"}]
	_payload = [ {"Panel Temp":_message[1]}, 
	             {"Water Temp":_message[2]},
	             {"Pump Speed":_message[3]},
	             {"Hours Pumped":_message[4]} ]
